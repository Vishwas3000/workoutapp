...
class RecentWorkoutsCard: UIView {
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            workoutsStackView.leadingAnchor.constraint(equalTo: containerView.leadingAnchor, constant: 20),
            workoutsStackView.trailingAnchor.constraint(equalTo: containerView.trailingAnchor, constant: -20),
            workoutsStackView.bottomAnchor.constraint(lessThanOrEqualTo: containerView.bottomAnchor, constant: -20),
        ...
        let bottomConstraint = workoutsStackView.bottomAnchor.constraint(equalTo: containerView.bottomAnchor, constant: -20)
    ...
    private func createWorkoutRow(for session: WorkoutSession) -> UIView {
        ...
        // Configure content
        if let workout = WorkoutService.shared.workoutPlan.first(where: { $0.id == session.workoutDayId }) {
            iconLabel.text = workout.type.icon
    ...
    @objc private func workoutRowTapped(_ gesture: UITapGestureRecognizer) {
        guard let view = gesture.view,
              view.tag < workoutSessions.count else { return }
